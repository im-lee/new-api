...
export function Wallet(props: WalletProps) {
  ...
  const emailReminderCheckedRef = useRef(false)
  ...
  useEffect(() => {
    if (userLoading || !user || emailReminderCheckedRef.current) return
    emailReminderCheckedRef.current = true

    const quota = Number(user.quota || 0)
    const quotaPerUnit = currency?.quotaPerUnit || 1
    const usdAmount = quota / quotaPerUnit
    const displayAmount =
    ...
    if (
      ...
      displayAmount > EMAIL_BIND_REMINDER_THRESHOLD &&
      !user.email?.trim()
    ) {
      setEmailBindReminderOpen(true)
    }
  }, [currency, user, userLoading])
  ...
  return (
    <>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('Email binding reminder')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                'To keep your account assets secure, we recommend binding an email address.'
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
